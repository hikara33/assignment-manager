...
describe('PriorityResolverService', () => {
  ...
  it('calculateScore: higher priority should have higher score', () => {
    ...
    expect(service.calculateScore(high)).toBeGreaterThan(
      service.calculateScore(low),
    );
  });
  ...
  it('calculateScore: tasks closer to deadline have higher score', () => {
    ...
    expect(service.calculateScore(near)).toBeGreaterThan(
      service.calculateScore(far),
    );
  });
  ...
  it('getPrioritizedAssignments: returns tasks sorted by score', async () => {
    ...
    const tasks: Assignment[] = [
      ...
      } as any,
      {
        id: 3,
        priority: 'MEDIUM',
        dueDay: new Date(now.getTime() + 2 * 24 * 60 * 60 * 1000),
        status: 'PENDING',
      } as any,
    ];
